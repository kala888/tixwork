...
public class MailAccount implements Serializable {
    ...
    /**
     * 设置发送方，遵循RFC-822标准<br> 发件人可以是以下形式：
     *
     * <pre>
     * 1. user@xxx.xx
     * 2.  name &lt;user@xxx.xx&gt;
    ...
    /**
     * 设置对于超长参数是否切分为多份，默认为false（国内邮箱附件不支持切分的附件名）<br> 注意此项为全局设置，此项会调用
     * <pre>
    ...
    /**
     * 设置对于文件名是否使用{@link #charset}编码，此选项不会修改全局配置<br> 如果此选项设置为{@code false}，则是否编码取决于两个系统属性：
     * <ul>
    ...
    public String toString() {
        return "MailAccount [host=" + host + ", port=" + port + ", auth=" + auth + ", user=" + user + ", pass=" + (
            StrUtil.isEmpty(this.pass) ? "" : "******") + ", from=" + from + ", startttlsEnable="
            + starttlsEnable + ", socketFactoryClass=" + socketFactoryClass + ", socketFactoryFallback=" + socketFactoryFallback
