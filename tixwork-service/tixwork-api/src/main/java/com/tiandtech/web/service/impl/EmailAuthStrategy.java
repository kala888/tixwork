...
import com.tiandtech.core.validate.auth.EmailGroup;
...
public class EmailAuthStrategy implements AuthStrategy {
    ...
    private boolean validateEmailCode(String tenantId, String email, String emailCode) {
        ...
        if (StringUtils.isBlank(code)) {
            loginService.saveLoginRecord(tenantId, email, Constants.LOGIN_FAIL, MessageUtils.message("user.jcaptcha.expire"));
    ...
    private SysUserVo loadUserByEmail(String tenantId, String email) {
        SysUser user = userMapper.selectOne(new LambdaQueryWrapper<SysUser>().select(SysUser::getEmail, SysUser::getStatus)
            .eq(TenantHelper.isEnable(), SysUser::getTenantId, tenantId).eq(SysUser::getEmail, email));
