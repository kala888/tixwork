...
import EleMoney from './ele-input/ele-money';
...
import EleMobileVerifyCode from './ele-input/ele-mobile-verify-code';
...
import EleNumberInput from '@/components/form/field/ele-input/ele-number-input';
...
export default function FlexField(props) {
  ...
  if (type === 'text' || type === 'email')
    return <EleInput placeholder='请输入' {...props} type='text' border={false} />;

  // double，decimal 应该在校验规则，精度上做手脚
  if (type === 'integer' || type === 'double' || type === 'decimal') return <EleNumberInput {...props} />;
  ...
  if (type === 'image') return <ImagePicker {...props} maxLength={props.max} />;

  //通过candidateValues来控制 多选，页面上有的多选Checkbox
  if (type === 'multi-select') return <ElePopupSelect {...props} multiple />;

  if (type === 'textarea' || type === 'long-text') return <Textarea {...props} />;
  ...
  // phone
  return <EleInput bordered={false} {...props} />;
}
