...
function ListofPageBase(props: listofPageBaseProps) {
  ...
  const initialHeight = () => {
    Taro.createSelectorQuery()
      ...
      .fields({ size: true }, (res) => {
        ...
        const footerHeight = ObjectUtils.isNotEmpty(props.actionList) || _.isFunction(renderFooter) ? 50 : 0;
  ...
  const theHeader = (
    <>
      {ObjectUtils.isNotEmpty(header) && <FlexLineItem item={header} {...header} />}
      {ObjectUtils.isNotEmpty(searchAction) && <FlexHeader {...searchAction} type='search' />}
      {ObjectUtils.isNotEmpty(content) && <FlexHeader item={{ content }} displayMode='rich-text' />}
      {ObjectUtils.isNotEmpty(tabs) && <FlexHeader type='tabs' tabs={tabs} />}
    </>
  );
