...
function FormItemLabel(props: FormItemLabelProps) {
  ...
  const hasTips = ObjectUtils.isNotEmpty(tips);
  ...
  return (
    <View className={rootClass} onClick={handleShowTips}>
      ...
      {ObjectUtils.isNotEmpty(tips) && <Text className='iconfont iconfont-question-circle' />}
      {tail}
      <FloatLayout visible={visible} onCancel={close}>
        <View className='form-item-label-tips'>
          ...
          <View className='form-item-label-tips-brief'>{_.get(tips, 'brief', tips as any)}</View>
        </View>
      </FloatLayout>
    </View>
  );
