...
// @ts-ignore
function listMerge(objValue, srcValue) {
...
export default {
  ...
  reducers: {
    ...
    saveInbound(state: ObjectPickerStateType, { payload }) {
      const inbound = _.mergeWith({}, state.inbound, { maxSelectCount: 20, ...payload }, listMerge)
      return ({
        ...state,
        inbound,
      })
