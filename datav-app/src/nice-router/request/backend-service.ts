...
class BackendServiceClass {
  ...
  async send(action: BackendServiceSendProps) {
    ...
    // mock 数据处理
    const mockData = await MockService.getMockResp(actionUri)
    if (mockData) {
      return mockData
    }

    if (OfflineService.isOfflineRequest(uri)) {
      return OfflineService.request(uri, params)
    }
    ...
    if (cache > 0 && isCacheable(resp)) {
      ...
      StorageTools.set(actionUri, resp, cache).then()
    }
    return resp
  }
